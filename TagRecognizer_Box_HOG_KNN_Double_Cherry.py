...
def tagPreprocessor_old(tid, imgid, image) :     	# tid is in range of 1,2,3,.... n template tags
...
def tagPreprocessor(tid, imgid, image) :     	# tid is in range of 1,2,3,.... n template tags
	#img1 = cv2.imread(image,0) 					# Read the image as GRAYSCALE
	#cv.NamedWindow('Original image')
	#cv2.imshow('Original image',img1)
	#print type(image), image
	#cv2.imshow('Image',image)
	blur = cv2.GaussianBlur(image,(5,5),0)	# Blur the image using GaussianBlur
	ret3,th3 = cv2.threshold(blur,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)	# Create binary image using OTSU thresholding
	imageinv =  255 - th3     				# Invert the image to have white text in black background
	kernel = np.ones((5,5),np.uint8)
	#erosion = cv2.erode(imageinv,kernel,iterations = 1) 	# Erode the image
...
def tagPreprocessor2_old(tid, imgid, image) :     	# tid is in range of 1,2,3,.... n template tags
	img1 = cv2.imread(image,0) 					# Read the image as GRAYSCALE
	#cv.NamedWindow('Original image')
	cv2.imshow('Original image',img1)
	blur = cv2.GaussianBlur(img1,(5,5),0)	# Blur the image using GaussianBlur
...
def computeSkewAngle(dilation):
	...
	try:
		...
		for i in range(len(contours[i])):
			cnt = contours[i]                      #### check why it is going list out of index  OR EXCEPTION HANDLING ######
			#print cnt
...
def rotateImage(skew_angle,box_vtx,dilation,image):
	...
	cv2.polylines(image,np.array([box_vtx],dtype=np.int32),True,(255,0,0),2)

	#erosioncopy = erosion[530:603,992:1037]
	#image = np.asarray(b[:,:])
	#cv2.imshow('Boxed image',erosion)
	cv2.imwrite('Z_Boxed_image.jpg',image)
...
def findExtremePoints(rot_image,rot_img_string):
	...
	for i in range(len(contours)):
		#print 'len of',i+1,'th contour :',len(contours[i])
		#print 'Area of', i+1, 'th contour :', cv2.contourArea(contours[i])
		#area_list[i] = cv2.contourArea(contours[i])
		if cv2.contourArea(contours[i]) > 1000:						# Discarding the small blobs
...
def Cropper(TagTemplates,resized,svm):
	OCRCharlist = []
	for i in range(TagTemplates[0][1]):
		print TagTemplates[0][1]
		crop_string,cropped = imgCropper(TagTemplates[0][i+2][1],TagTemplates[0][i+2][2],TagTemplates[0][i+2][3],TagTemplates[0][i+2][4],resized,TagTemplates[0][i+2][0])
...
def JSONBuilder(newOCRlist, TagTemplates):
	nlist = []
	for i in range(TagTemplates[0][1]):
		nlist.append(TagTemplates[0][i+2][0])
...
TagTemplates = [
					[
						...
						['T_unk2',330,1100,490,660],
						['T_unk3',1800,2500,490,660],
						['T_dob',10,1100,330,500],
						['T_name',10,2000,190,330],						
...
def mainProcess1(tid,image):
	
	g = open("insideMainProcessCherrypy.jpg", "w")
	g.write(base64.decodestring(image))
	g.close()
	imgid = 111111
	image = cv2.imread('insideMainProcessCherrypy.jpg',0)
	print 'Image type in MainProcess  : ',type(image)
	dilation = tagPreprocessor(tid,imgid,image)   # STEP 1 : calling the preprocessor for full dilated input image
	...
	rot_image, rot_img_string = rotateImage(skew_angle,box_vtx,dilation,image)  # STEP 3 : rotate the image with skew angle
	print type(rot_image)
	...
	try:
		...
		image_file = 'Z_Resized_color_image.png'
		image = image_to_base64(image_file)
		image = cv2.imread('Z_Resized_color_image.png',0)
		...
		# 	model.train(samples,responses)
			...
			OCRCharlist = Cropper(TagTemplates,resized,svm)# STEP 8 : Crop every placeholder with cropping parameters from TagTemplates list
