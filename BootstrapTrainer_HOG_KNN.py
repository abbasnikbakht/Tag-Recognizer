...
img = cv2.imread('111_cut.jpg')
print img.shape
img1 = cv2.imread('111_cut.jpg',0) 					# Read the image as GRAYSCALE
    ...
    if cv2.contourArea(cnt)>500:
        ...
        if  h>10:
			...
			if key == 27:  # (escape to quit)
				...
				#######  Introduce HOG function here for feature extraction #########
				#hogdata = hog(roismall)
				hogdata_reshaped = np.float32(hogdata).reshape(-1,64)
				hogData = np.append(hogData,hogdata_reshaped,0)
