...
def tagPreprocessor_HSV(tid,image):
	print 'Type of image sent from client is :',type(image)
	...
	hsv_img = cv2.cvtColor(image,cv2.COLOR_BGR2HSV)	# Convert BGR to HSV color space
...
# 	cv2.imwrite('CLAHEEqualizedOTSU.jpg',otsu)
	...
	dilation_string = 'CLAHEDilatedImage.jpg'
	return dilation,dilation_string #imageinv #erosion
...
def rotateImage(skew_angle,box_vtx,dilation,image):
...
def Cropper(TagTemplates,tid,resized_dilation,svm):
	OCRCharlist = []
	...
	for i in range(TagTemplates[index][1]):
		...
		crop_string,cropped = imgCropper(TagTemplates[index][i+4][1],TagTemplates[index][i+4][2],TagTemplates[index][i+4][3],TagTemplates[index][i+4][4],resized_dilation,TagTemplates[index][i+4][0])
		...
		OCRAsciilist = OCR_new(sortContoursList,cropped,svm)          # STEP 10 : Recognize every character present in sortContoursList of every placeholder
...
def OCR_new(sortContoursList,cropped,svm):
...
def cleanTessOCR(OCRlist,tagIndex):
	#if OCRlist[0]
	# if any letters found in OCRlist[0] mrn => then change it to its corresponding digits say, b to 6, q to 9, l to 1, so on
	# if any letters found in OCRlist[1] unk1 => then change it to its corresponding digits as above
	numOCRlist =[]
	...
	lst_num = []
	lst_alpha = []
	num_index = TagTemplates[tagIndex][2]	#[0,1,4,6,7]
	alpha_index = TagTemplates[tagIndex][3]	#[2,3,5,8,9]
	for i in num_index:
		lst_num.append(OCRlist[i])
	for i in alpha_index:
		lst_alpha.append(OCRlist[i])
	
	#nOCRlist = OCRlist[:5]  # splitting the digits placeholders
	print 'Inside cleanTess',lst_num
	print lst_num
	for j in range(len(lst_num)):
		...
		#posq = (OCRlist[0]).find('q')
		posq = [m.start() for m in re.finditer('q',newstr)]
		posI = [m.start() for m in re.finditer('I',newstr)] # I to 1
		# finds all occurrences of a single character
		#print posq
		posB = [m.start() for m in re.finditer('B',newstr)] # B to 8
		posl = [m.start() for m in re.finditer('l',newstr)] # l to 1
		poso = [m.start() for m in re.finditer('o',newstr)] # o to 0
		posexcl = [m.start() for m in re.finditer('!',newstr)]
		#pospipe = [m.start() for m in re.finditer('|',newstr)]   # pipe is not working as it is matching with 1
...
# 		
	...
	# 			#newOCRlist.append(newstr)
		...
		alphaOCRlist.append(newstr)
	
	
	
	print alphaOCRlist
	return numOCRlist, alphaOCRlist, num_index, alpha_index
...
def JSONBuilder(newOCRlist, TagTemplates,tagIndex):
	...
	for i in range(TagTemplates[tagIndex][1]):
		nlist.append(TagTemplates[tagIndex][i+4][0])
...
def tagRecognizer_old(croplist):
	OCRlist = []
	#print OCRlist
	for i in range(len(croplist)):
		print len(croplist)
...
def tagRecognizer(resized_dilation_string,api):
	print ' 4 & inside tagRecognizer'
	OCRlist = []
	#print OCRlist
	image0 = cv2.imread(resized_dilation_string)
	#image=cv.LoadImage('CLAHEDilatedImage.jpg')
	imagen= cv2.imread('Z_Rotated_color_image.jpg',0)
	kernel = np.ones((5,5),np.uint8)
	image0 = cv2.dilate(image0,kernel,iterations = 1)	# Dilate the image
	cv2.imwrite('HSV_DoubledilatedImage.jpg',image0)
	
	image1=cv.LoadImage('HSV_DoubledilatedImage.jpg')
	print ' 5'
	tesseract.SetCvImage(image1,api)
	print ' 6'
	OCR=api.GetUTF8Text()
...
TagTemplates = [
					[
						...
						[2,3,5,8,9],
						['MRN',470,1500,1,195],
						['Unk1',1800,2500,1,190],
						['Unk2',330,1400,490,660],
						['Unk3',1800,2500,490,660],
						['DOB',1,1100,330,500],
						['Name',1,2000,190,330],						
						['Sex',1600,1800,330,510],
						['Doctor',415,2000,645,800],
					],
					[
						'222',
						11,
						...
						['PFName',1,2000,1,100],
						['DOB',1,1000,100,250],
						['Sex',1000,1300,120,250],
						['MRN1',1300,2500,120,250],
						['MBNo',1,1750,230,380],
						['Dept',1650,2500,250,430],
						['DLName',1,2000,400,500],
						['DFName',1,2000,400,500],
						['MRN2',800,2500,520,700],
						['Lab',50,1500,700,800],
					]
...
# 	cv2.imwrite('output2.jpg', frame_threshed)
	...
	rot_image, rot_color_image, rot_img_string = rotateImage(skew_angle,box_vtx,dilation,image)  # STEP 3 : rotate the image with skew angle
	#print type(rot_image)
	...
	try:
		...
		resized_dilation,resized_dilation_string = tagPreprocessor_HistEqualize(resized_color_image_string)
	...
	# 	model.train(samples,responses)
		...
		OCRCharlist,tagIndex = Cropper(TagTemplates,tid,resized_dilation,svm)# STEP 8 : Crop every placeholder with cropping parameters from TagTemplates list
		...
		#import tesseract
		######## NEW CODE FOR TESSERACT API INTEGRATION AND MATCHING AS WELL #######
		api = tesseract.TessBaseAPI()
		print ' 1'
		api.Init(".","sti",tesseract.OEM_DEFAULT)
		print ' 2'
		#api.SetPageSegMode(tesseract.PSM_AUTO)
		api.SetPageSegMode(6)
		print ' 3'
		
		TesseractOCRlist = tagRecognizer(resized_dilation_string,api)
		print 'OUT OF TESSERACT MODULE', TesseractOCRlist
		
		###### parsing tesseract's output #######
		tess_list = TesseractOCRlist.split('\n')
		print tess_list
		
		new = []
		for i in range(len(tess_list)):
			new.append(tess_list[i].split(' '))
		print 'NEW:    ',new
		...
		#######  Cleaning is called here #######
		cleanednumOCRlist, cleanedalphaOCRlist, num_index, alpha_index = cleanTessOCR(new1,tagIndex)		
		
		###### Merging the cleaned outputs together #######
		
		#cleanedTessOCRlist = num_index aplha_index numOCRlist alphaOCRlist
		
		numlist = {}
		numlist = dict(zip(num_index, cleanednumOCRlist))
		print numlist
		
		alphalist = {}
		alphalist = dict(zip(alpha_index, cleanedalphaOCRlist))
		print alphalist

		
		merged_dict = numlist.copy()
		#merged_dict = {}
		merged_dict.update(alphalist)
		print merged_dict
		
		cleanedTessOCRlist = dict(sorted(merged_dict.items()))
		#newnumlist = make a list of tuples with (num_index,numOCRlist)
		# make another list of tuples with (alpha_index,alphaOCRlist)
		# merge two lists together ans sort them with index values	
		#write a taglist function that takes TagNames from the TagTemplates List
		taglist = []
		print new1[2]
		merged_dict.update({2:new1[2]})
		print merged_dict
		for i in range(TagTemplates[tagIndex][1]):
			taglist.append(TagTemplates[tagIndex][i+4][0])
		#taglist = ['MRN','UNK1','PLName','PFName','DOB','SEX','MBNo','UNK2','DLName','DFName']
		tesslst = []
		print taglist
		for i in range(len(merged_dict)):
			d = {}
			d[taglist[i]] = merged_dict[i]
			tesslst.append(d)
			tess_json_string = json.dumps(tesslst)
		print tess_json_string
		
	return json_string, tess_json_string
	
	
	# visa 4520010040968242
# 	valid form sep 2013   valid till sep 2016
# 	vejey karthick gandier
# 	663
# 	Billing address: 806 (apt no)-11(building no), Michael Power Place(street name), Toronto ONTARIO M9A5G3
