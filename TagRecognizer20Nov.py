...
def tagPreprocessor(tid, imgid, image) :     	# tid is in range of 1,2,3,.... n template tags
	...
	cv2.imwrite('ThreshBlurred.jpg',blur)
	ret, otsu = cv2.threshold(blur,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
	cv2.imwrite('ThreshCLAHEEqualizedOTSU.jpg',otsu)

	# blur = cv2.medianBlur(img1,5)	# Blur the image using GaussianBlur
# 	cv2.imwrite('BlurredImage.jpg',blur)
...
# 	#ret3,th3 = cv2.threshold(blur,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)	# Create binary image using OTSU thresholding
	cv2.imwrite('ThresholdedImage.jpg',otsu)
	imageinv =  255 - otsu     				# Invert the image to have white text in black background
	cv2.imwrite('InvertedImage.jpg',imageinv)
	kernel = np.ones((5,5),np.uint8)
	erosion = cv2.erode(imageinv,kernel,iterations = 2) 	# Erode the image
	#cv2.imwrite('ErodedImage.jpg',erosion)
	#dilation = cv2.dilate(imageinv,kernel,iterations = 1)	# Dilate the image
	#cv2.imwrite('DilatedImage.jpg',dilation)
	return erosion #imageinv #dilation
...
# 	cv2.imwrite('output2.jpg', frame_threshed)
	...
	rot_image, rot_img_string = rotateImage(skew_angle,box_vtx,dilation)  # STEP 3 : rotate the image with skew angle
	print type(rot_image)
	...
	try:
		...
		'''
		skew_angle,box_vtx = computeSkewAngle(cropppped)
