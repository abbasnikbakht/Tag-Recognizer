	...
	try:
		...
		for i in range(len(contours[i])):
			...
			if cv2.contourArea(cnt) > 10000:
				print 'Found bigggggg contour'                   #### check why it is going list out of index  OR EXCEPTION HANDLING ######
			#print cnt
			else:
...
def findExtremePoints(rot_image,rot_img_string):
	...
	for i in range(len(contours)):
		...
		if cv2.contourArea(contours[i]) > 10000:		# Discarding the small blobs
			print 'Found Big contour'
		elif cv2.contourArea(contours[i]) > 2000:					
